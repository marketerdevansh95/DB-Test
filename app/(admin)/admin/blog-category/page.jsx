"use client";

import Loader from "@/FCOMPS/Loader";
import DeleteModal from "@/dashboardComponent/DeleteModal";
import QuickEditModal from "@/dashboardComponent/QuickEditModal";
import DeleteIcon from "@/utils/DeleteIcon";
import EditIcon from "@/utils/EditIcon";
import Link from "next/link";
import React, { useEffect, useState } from "react";
import { ToastContainer, toast, Bounce } from "react-toastify";

const BlogCategoryList = () => {
  const [loading, setLoading] = useState(true);
  const [blog, setBlog] = useState([]);
  const [showSearch, setShowSearch] = useState(false);
  const [selectedItemId, setSelectedItemId] = useState(null);
  const [quickEditId, setQuickEditId] = useState(null);
  const [quickEditData, setQuickEditData] = useState({});

  const handleQuickEdit = (e, id) => {
    e.preventDefault();
    setQuickEditId(id);
    setQuickEditData(blog.find((item) => item._id === id));
  };

  const handleConfirmQuickEdit = async (data) => {
    const response = await fetch(
      `${process.env.BASE_URL}/api/blog-category-routes/b/quick-edit-blog-category/${quickEditId}`,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      }
    );
    const result = await response.json();

    if (result.status === 200) {
      toast.success("Blog Category Meta Updated Successfully", {
        theme: "colored",
      });
      setBlog((prevBlog) =>
        prevBlog.map((item) =>
          item._id === quickEditId ? { ...item, ...data } : item
        )
      );
    }

    setQuickEditId(null);
  };

  const handleDelete = (e, id) => {
    e.preventDefault();
    setSelectedItemId(id);
  };

  const handleConfirmDelete = async () => {
    const response = await fetch(
      `${process.env.BASE_URL}/api/blog-category-routes/b/delete-blog-category/${selectedItemId}`,
      { method: "DELETE" }
    );
    const result = await response.json();

    if (result.status === 200) {
      setBlog((prevBlog) =>
        prevBlog.filter((item) => item._id !== selectedItemId)
      );
      toast.success("Item Deleted Successfully");
    }

    setSelectedItemId(null);
  };

  const handleSearch = async (e) => {
    const query = e.target.value;
    const response = await fetch(
      `${
        process.env.BASE_URL
      }/api/blog-category-routes/b/get-all-blog-category/${
        query === "" ? "all" : query
      }`
    );
    const result = await response.json();
    setBlog(result.data);
    setLoading(false);
  };

  useEffect(() => {
    handleSearch({ target: { value: "" } });
  }, []);

  return (
    <>
      <ToastContainer
        position="top-right"
        autoClose={5000}
        hideProgressBar={false}
        newestOnTop={false}
        closeOnClick={false}
        rtl={false}
        pauseOnFocusLoss
        draggable
        pauseOnHover
        theme="light"
        transition={Bounce}
      />
      {selectedItemId && (
        <DeleteModal
          onConfirm={handleConfirmDelete}
          onCancel={() => setSelectedItemId(null)}
        />
      )}

      {quickEditId && (
        <QuickEditModal
          onUpdate={handleConfirmQuickEdit}
          onCancel={() => setQuickEditId(null)}
          data={quickEditData}
        />
      )}

      <div className="container wrapper">
        <div className="row">
          <div className="wrapper-top">
            <div>
              <h1>Blog Category List</h1>
            </div>
            <div className="wrapper-search">
              <ul className="all-search">
                <li className="form-group m-0">
                  <input
                    type="text"
                    id="search"
                    className={`form-control m-0 search ${
                      showSearch ? "searchafter" : ""
                    }`}
                    placeholder="Search"
                    onChange={handleSearch}
                  />
                </li>
                <li className="search-icon">
                  <span
                    onClick={() =>
                      setShowSearch((prevShowSearch) => !prevShowSearch)
                    }
                  >
                    {showSearch ? (
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        height="1em"
                        viewBox="0 0 384 512"
                      >
                        <path d="M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" />
                      </svg>
                    ) : (
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        height="1em"
                        viewBox="0 0 512 512"
                      >
                        <path d="M416 208c0 45.9-14.9 88.3-40 122.7L502.6 457.4c12.5 12.5 12.5 32.8 0 45.3s-32.8 12.5-45.3 0L330.7 376c-34.4 25.2-76.8 40-122.7 40C93.1 416 0 322.9 0 208S93.1 0 208 0S416 93.1 416 208zM208 352a144 144 0 1 0 0-288 144 144 0 1 0 0 288z" />
                      </svg>
                    )}
                  </span>
                </li>
                <li>
                  <Link href="/admin/blog-category/create-blog-category">
                    Create
                  </Link>
                </li>
              </ul>
            </div>
          </div>
        </div>
      </div>

      {loading ? (
        <Loader />
      ) : (
        <div className="row">
          {blog.map((data) => (
            <div className="col-12" key={data._id}>
              <div className="brand-list">
                <div>{data.name}</div>
                <div className="brand-actions">
                  <Link href="#" onClick={(e) => handleQuickEdit(e, data._id)}>
                    Quick Edit
                  </Link>
                  <Link href={`/admin/blog-category/${data._id}`}>
                    <EditIcon />
                  </Link>
                  <Link href="#" onClick={(e) => handleDelete(e, data._id)}>
                    <DeleteIcon />
                  </Link>
                </div>
              </div>
              <div className="brand-meta">
                <p>{data.metaTitle}</p>
                <p>{data.metaDescription}</p>
              </div>
            </div>
          ))}
        </div>
      )}
    </>
  );
};

export default BlogCategoryList;
