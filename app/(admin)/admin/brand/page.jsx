...
const ShowBrands = () => {
  ...
  const handleChange = async (event) => {
    ...
    const request = await fetch(
      `${process.env.BASE_URL}/api/brand-routes/b/category-based-brands/${selectedValue}`,
      {
        method: "POST",
      }
    );
  ...
  const handleQuickEdit = (e, id) => {
    e.preventDefault();
    setQuickEditId(id);
    setQuickEditData(brands.find((item) => item._id == id));
  ...
  const handleConfirmQuickEdit = async (data) => {
    ...
    const quick_edit_req = await fetch(
      `${process.env.BASE_URL}/api/brand-routes/b/quick-edit-brand/${quickEditId}`,
      {
        method: "PATCH",
    ...
    if (quick_edit_res.status === 200) {
      toast.success("Brand Meta Updated Successfully", {
        theme: "colored",
      });
      setBrands(
        brands.map((item) =>
  ...
  const handleDelete = (e, id) => {
    ...
    // console.log("selected id set for delete");
  };

  const handleConfirmDelete = async (e) => {
    e.preventDefault();
    const del_res = await fetch(
      `${process.env.BASE_URL}/api/brand-routes/b/delete-brand/${selectedItemId}`,
      {
        method: "DELETE",
      }
    )
    ...
    if (del_res.status === 200) {
      ...
      setBrands(brands.filter((item) => item._id != selectedItemId));
    }
  ...
  const handleActive = async (e, id) => {
    ...
    const active_res = await fetch(
      `${process.env.BASE_URL}/api/brand-routes/b/make-brand-active/${id}`,
      {
        method: "PATCH",
      }
    )
  ...
  const handleInActive = async (e, id) => {
    ...
    const inactive_res = await fetch(
      `${process.env.BASE_URL}/api/brand-routes/b/make-brand-inactive/${id}`,
      {
        method: "PATCH",
      }
    )
  ...
  const handleDiscard = async (e) => {
    ...
    setDiscard(true);
    await fetch(
      `${process.env.BASE_URL}/api/brand-routes/b/get-discarded-brands`
    )
      .then((res) => res.json())
      .then((data) => {
        setLoading(false);
  ...
  const handleSearch = async (e, no) => {
    await fetch(
      `${process.env.BASE_URL}/api/brand-routes/b/get-all-brand-list/${
        e === "" ? "all" : e
      }&page=${no}`
    )
      .then((res) => res.json())
      .then((data) => {
        setCount(Math.ceil(parseInt(data.brand_count) / 50));
        setBrands(data.data);
        setPage(no);
      });
  };
    ...
    <>
          ...
          <div className="wrapper-top">
            ...
            <div className="wrapper-search">
              <ul className="all-search">
                ...
                <li>
                  <Link href={"/admin/brand/create-brand"} passHref>
                    Create
                  </Link>
                </li>
                <li>
      ...
      {loading ? (
        ...
        <div className="row wrapper">
          <div className="col-4">
            <select onChange={handleChange} value={selectedCategory}>
              ...
              {categoryList.map((item) => {
                return (
                  <option key={item.path} value={item._id}>
                    {item.name}
                  </option>
                );
              })}
          ...
          {brands.map((data) => {
            return (
              <div className="col-12 seprator" key={data._id}>
                <div className="brand-list">
                  <div>{data.name}</div>
                  <div className="brand-actions">
                    <Link href={`/admin/brand/products/${data.path}`}>
                      Products
                    </Link>
                    <Link
                      href={"#"}
                    ...
                    <Link href={`/admin/brand/${data._id}`} passHref>
                      <EditIcon />
                    </Link>
                    <Link
                      href={"#"}
                ...
                <ul className="brand-cat">
                  {!discard &&
                    data.category.map((item) => {
                      return <li key={data._id + item._id}>{item.name}</li>;
                    })}
                </ul>
        ...
        <div className="row justify-content-center mb-lg-5 mb-3">
          <div className="pagination pg-admin">
            {prev && (
              <>
                <Link
                  href={`brand?page=${parseInt(page) - 1}`}
