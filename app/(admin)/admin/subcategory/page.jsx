...
const ShowAllSubCategory = () => {
  ...
  const handleQuickEdit = (e, id) => {
    e.preventDefault();
    setQuickEditId(id);
    console.log(allCategory.find((item) => item._id == id))
  ...
  const handleConfirmQuickEdit = async (data) => {
    console.log(data);
    console.log(quickEditId)
    const quick_edit_req = await fetch(
      `${process.env.BASE_URL}/api/subcategory-routes/b/quick-edit-subcategory/${quickEditId}`,
      {
        method: "PATCH",
        headers: {
  ...
  const handleConfirmDelete = async (e, id) => {
    ...
    const del_res = await fetch(
      `${process.env.BASE_URL}/api/subcategory-routes/b/delete-subcategory/${selectedItemId}`,
      {
        method: "DELETE",
      }
    )
  ...
  const handleSearch = async (e, no) => {
    setLoading(true);
    await fetch(
      `${process.env.BASE_URL}/api/subcategory-routes/b/get-all-subcategory-list/${
        e === "" ? "all" : e
      }&page=${no}`
    )
      .then((res) => res.json())
          ...
          <div className="wrapper-top">
            ...
            <div className="wrapper-search">
              <ul className="all-search">
                ...
                <li>
                  <Link href={"/admin/subcategory/create-subcategory"} passHref>
                    Create
                  </Link>
                </li>
              </ul>
              ...
              <div className="col-12" key={data._id}>
                <div className="brand-list">
                  ...
                  <div className="brand-actions">
                    ...
                    <Link href={`/admin/subcategory/${data._id}`} passHref>
                      <EditIcon />
                    </Link>
                    <Link
                      href={"#"}
